object LearningScala1 {
  ...
  val moreStuff = immutableHelloThere + "yeah"    //> Hola!There!yeah
  // Scala can infer types
  ...
  // printf style (formatting print statements):
  println(f"Pi is about $piSinglePrecision%.3f")  //> Pi is about 3.142 (.3f specifies floating point value with 3 decimal places precision)
  println(f"Zero padding on the left: $numberOne%05d")
                                                  //> Zero padding on the left: 00001 (d = integer value)
  ...
  val theUltimateAnswer: String = "To life, the universe, and everything is 42."
                                                  ...
                                                  //| 
  val pattern = """.* ([\d]+).*""".r              //> pattern  : scala.util.matching.Regex = .* ([\d]+).* (.r denotes regular expression (everything between the triple quotes))
  ...
  println(f"Here is the value of pi doubled: ${3.14159265 * 2}%.3f")  //> Here is the value of pi doubled: 6.283
}