object LearningScala4 {
  ...
  // Immutable lists - cannot change once they have been created
  ...
  // You refer to individual fields with their ONE-BASED (not starting at zero) index:
...
val iHateThrees = numberList.filter(_ != 3)       //> iHateThrees  : List[Int] = List(1, 2, 4, 5) -- shorthand for checking whatever parameter element comes through
...
//  val divThreeList = List.range(1, 20)
//  for (num <- divThreeList) {
//    if (num % 3 == 0) {
//      println(num)
//    }
//  }
  ...
  val divThreeList = List.range(1, 20)
  println(divThreeList.filter(num => num % 3 == 0))
