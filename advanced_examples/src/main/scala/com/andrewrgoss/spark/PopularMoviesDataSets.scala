...
object PopularMoviesDataSets {
  ...
  def main(args: Array[String]) {
    ...
    val lines = spark.sparkContext.textFile("../advanced_examples/datasets/ml-100k/u.data").map(x => Movie(x.split("\t")(1).toInt))
    
    // Convert to a DataSet
    import spark.implicits._
    val moviesDS = lines.toDS()
    
    // Some SQL-style magic to sort all movies by popularity (defined as number of times a movie was rated) in one line!
