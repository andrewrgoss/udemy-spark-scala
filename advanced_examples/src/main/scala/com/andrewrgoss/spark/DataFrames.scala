...
object DataFrames {
  ...
  def main(args: Array[String]) {
    ...
    val people = lines.map(mapper).toDS().cache() // Cache in memory to avoid reconstructing dataset more than once.
    // 'People' is a dataset of person objects.
    ...
    people.select("name").show() // Show() function - quickly display top 20 results in dataset.
