...
object LinearRegressionDataFrame {
  ...
  def main(args: Array[String]) {
    ...
    val inputLines = spark.sparkContext.textFile("../advanced_examples/datasets/regression.txt")
    val data = inputLines.map(_.split(",")).map(x => (x(0).toDouble, Vectors.dense(x(1).toDouble)))
    // Underscore '_' represents a wild card value (each input coming into map function gets split)
