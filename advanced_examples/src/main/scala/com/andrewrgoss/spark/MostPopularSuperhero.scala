...
object MostPopularSuperhero {
  ...
  def countCoOccurences(line: String) = {
    val elements = line.split("\\s+") // Regular expression to split on whitespace
    (elements(0).toInt, elements.length - 1)
  }
  
  // Function to extract hero ID -> hero name tuples (or None in case of failure)
  def parseNames(line: String) : Option[(Int, String)] = { // Option is a Scala construct for null/nil value
  ...
  def main(args: Array[String]) {
    ...
    val names = sc.textFile("../advanced_examples/datasets/marvel-names.txt")
    val namesRdd = names.flatMap(parseNames)
    
    // Load up the superhero co-appearance data
    val lines = sc.textFile("../advanced_examples/datasets/marvel-graph.txt")
